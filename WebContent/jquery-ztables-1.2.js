(function ( $ ) {
	...
	$.fn.ZTable = function(options) {
		...
		var __table = {columns: new Array(), rows: new Array()};  // The table data
		...
		this.setPage = function(pgNum){
			...
			filteredRows.forEach(function(row, rowIndex){ 
				...
				__table.columns.forEach(function(col, colIndex){
					if (col.show)
						tr.append(renderCell(row[col.id], colIndex, rowIndex, row));
					else
						tr.append($('<td>'));
				});
				bod.append(tr);
		...
		/*
		 * Returns the index of a column with an ID of 'colid'
		 */
		function getColumnById(colid){
			for(i=0;i<__table.columns.length;i++){
				if (__table.columns[i].id == colid)
					return i;
			}
			return -1;
		}
		...
		this.setColumnVisible = function(colId, state){
			__table.columns[getColumnById(colId)].show = state;
			buildHeaders();
			that.setPage(pageNumber);
		}
